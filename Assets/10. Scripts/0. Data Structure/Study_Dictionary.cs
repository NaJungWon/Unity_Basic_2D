using System.Collections.Generic; // 자료 구조사용하려면 해당 네임스페이스를 사용해야 합니다.
using UnityEngine;
...
namespace Study_DataStructure
{
    public class Study_Dictionary : MonoBehaviour
    {
        ...
        private void StudyDictionary()
        {
            ...
            // 같은 Key로 데이터를 추가해보면?
            // => 같은 이름표를 가진 다른값을 추가해본다면?
            //studentAge.Add("김철수", 25);
            // ArgumentException 오류가 발생합니다. Key는 고유해야 합니다
            // Dictionary의 Key는 Hash입니다. 그래서 고유함이 보장되어야 합니다
            ...
            // 없는 Key로 데이터에 접근하면?
            //Debug.Log($"최수지의 나이 : {studentAge["최수지"]}"); // "22"
            ...
            Debug.Log($"이영희 학생정보가 있나요? {studentAge.ContainsKey("이영희")}");
            Debug.Log($"최수지 학생정보가 있나요? {studentAge.ContainsKey("최수지")}");
