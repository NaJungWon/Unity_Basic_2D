...
namespace Study_DataStructure
{
    ...
    // 1. 선입선출(FIFO - First In, First Our) : 가장 먼저 줄을 선 사람이 가장 먼저
    // 서비스를 받는 구조. 새로운 데이터는 줄의 맨 뒤에 추가되고,
    // 데이터를 꺼낼때는 줄의 맨 앞에서 꺼냅니다.
    ...
    public class Study_Queue : MonoBehaviour
    {
        ...
        private void Queue()
        {
            ...
            Debug.Log($"줄의 맨 앞 사람 : {waitingLine.Peek()}");
            Debug.Log($"대기열의 사람 수 : {waitingLine.Count}");
            Debug.Log("");
            ...
            // - 운영체제의 작업 스케쥴링 : CPU가 철리해야할 작업들이 Queue에 쌓여 순서대로 처리됩니다.
            // - 온라인게임의 접속 대기열 : 먼저 접속을 대기한 유저부터 게임에 입장합니다.
            // 2. 버퍼링 (Buffering)
            // - 데이터를 일시적으로 저장하고 순서대로 처리해야 할 때 사용됩니다.
            // 예를 들어, 네트워크에서 데이터를 수신할 때.
            // 3. 너비 우선 탐색 (BFS - Breadth-First Search)
            // - 그래프나 트리를 탐색할 때, 현재 노드와 연결된 모든 노드를 먼저 방문하고
            // 다음 깊이로 넘어가는 BFS알고리즘에서 사용합니다.
