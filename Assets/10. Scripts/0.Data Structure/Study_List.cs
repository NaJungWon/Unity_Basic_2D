...
// - C#에서 지원하는 자료구조 도구들을 사용할 수 있는 네임스페이스입니다.
using UnityEngine;
...
namespace Study_DataStructure
{
    public class Study_List : MonoBehaviour
    {
        ...
        //    알아서 늘어납니다. 반대로 물건을 빼면 크기가 줄어듭니다.
        //    배열(Array)처럼 크기를 미리 정해둘 필요가 없음.

        // 2. 한 종류만 담는 규칙 : 이 쇼핑백은 특별해서, "과일만 담는 쇼핑백", "장난감"
        //    만 담은 쇼핑백 처럼 한종류의 물건만 담기로 약속해야 합니다.
        //    만약, 숫자를 담는 LIst를 만들었다면
        //    문자열이나 다른 타입의 데이터는 넣을 수 없습니다.
        ...
        private void List()
        {
            ...
            // .Count : 리스트 객체가 현재 가지고 있는 데이터 갯수를 반환합니다.
            Debug.Log("");
            ...
            Debug.Log($"2. 과일 3개 추가 직후");
            Debug.Log($"쇼핑백 안의 과일 개수 : {fruitBasket.Count}");
            Debug.Log("");

            // # 3. 데이터 접근 : 인덱스(index)사용
            // List 안의 데이터들은 배열과 마찬가지로 0번부터 순서대로 번호표를 받습니다.
            // 이 번호를 "index"라고 부릅니다.
            // fruitBasket[0]은 첫번째 데이터, 
            // fruitBasket[1]은 두번째 데이터, 배열과 동일합니다.

            Debug.Log("3. 과일에 접근해보기");
            Debug.Log($"fruitBakset[1] = {fruitBasket[1]}");
            Debug.Log("");
            ...
            {
                Debug.Log($"fruitBakset[{i}] = {fruitBasket[i]}");
            }
            Debug.Log("");
            ...
            // .Insert() : 특정 인덱스에 값을 추가하고 리스트를 뒤로 한칸 밀어냅니다.
            ...
            // 2. 순서가 중요 할 때:
            //    List<T>는 데이터가 들어온 순서대로 차곡차곡 관리 됩니다.
            //    특정 인덱스의 삽입/삭제를 할때도 순서대로 관리 됩니다.
        }
    }
}