...
namespace Study.PrimitiveAndVector
{
    ...
    //              2D Vector는 3D Vector의 하위 개념이어서 Vector3만 사용하면
    //              2D도 자동적으로 사용 가능합니다.
    
    // # Vector? 
    // 3차원 공간에서 위치, 방향(or 크기)를 나타내는 데 사용되는 구조체(Struct)
    // 마치, 지도에서 특정 지점의 좌표를 표시하거나, 어떤 방향으로 어떤속도 만큼
    // 이동하는지를 표현하는데에 사용됩니다.
    // Vector는 게임엔진의 모든 3D 계산의 기본이 되며, GameObject의 위치, 이동 방향
    ...
    public class Study_Vector : MonoBehaviour
    {
        ...
        // # 기본 단위
        // Vector3.zero : (0,0,0)을 담아둔 정적 변수(원점, 크기가 없는 상태)
