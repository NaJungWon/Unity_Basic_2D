...
namespace Study.MergeGame
{
    public class MergeGameController : MonoBehaviour
    {
        ...
        private void UpdateMovement()
        {
            ...
            // 게임내의 좌표계로 바꿔줍니다
            Vector3 moveVector = new Vector3(mouseDelta.x, 0.0f, 0.0f) * speed * Time.deltaTime;

            Vector3 adjustPosition = 
                transform.position + moveVector;
            adjustPosition.x = Mathf.Clamp(adjustPosition.x, minXPosition, maxXPosition);
            // x가 구간은 벗어나지 않게 하는 값
        ...
        {
            // MargeGameManager에서 공을 가지고 와서
            // SpwanPoint안에 생성을 합니다.

            AnimalBall spawnBall = MergeGameManager.Instance.GetNextBall();
            // MergeGameManager에서 가져온 프리팹을 이용해서 새로운 볼을 만들어준다
            // Instantiate(견본, Transform 매개변수) :
            // 매개변수로 전달된 Trasnform의 자식으로 복사본을 생성합니다.
